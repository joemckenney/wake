...
impl Model {
    ...
    pub async fn load(model_path: &Path) -> Result<Self, ModelError> {
        ...
        let inner = GgufModelBuilder::new(model_path_str)
            .with_logging()
            .build()
            .await
    ...
    pub async fn load(model_path: &Path) -> Result<Self, ModelError> {
        ...
        Ok(Self {
            _path: model_path.to_path_buf(),
        })
    }

    /// Run inference on the model with the given messages
    #[cfg(feature = "llm")]
    pub async fn generate(&self, system_prompt: &str, user_message: &str) -> Result<String, ModelError> {
    ...
    #[cfg(not(feature = "llm"))]
    pub async fn generate(&self, _system_prompt: &str, _user_message: &str) -> Result<String, ModelError> {
